...
const HakAksesTable = () => {
    ...
    const { data, loading } = useHakAkses()
                ...
                <Table>
                    ...
                    <TableBody>
                        {rows.map((row) => (
                            <TableRow
                                key={row.id}
                                sx={{ 'td, th': { borderRight: '2px solid rgba(0, 0, 0, 0.12)' } }}
                            >
                                <TableCell component="th" scope="row" align="center" sx={{width: '20%'}}>
                                    {row.kode}
                                </TableCell>
                                <TableCell align="center" sx={{width: '60%'}}>{ row.nama }</TableCell>
                                <TableCell align="center" sx={{width: '10%'}}>{ row.status }</TableCell>
                                <TableCell align="center" sx={{width: '10%'}}><UbahHakAkses /></TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
